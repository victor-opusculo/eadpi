  ...
  import { h, Component } from './lego.min.js'
    ...
    const methods =
    {
        ...
        submit(e)
        {
            ...
            fetch(EADPI.Helpers.URLGenerator.generateApiUrl('/student/' + this.state.studentid), { method: 'PUT', headers, body })
            .then(res => res.json())
            .then(json =>
            {
    ...
    h("form", {"class": `mx-auto max-w-[700px]`, "onsubmit": this.submit.bind(this)}, [
      ...
      h("ext-label", {"label": `Seu fuso horário`}, [
        h("select", {"onchange": this.timezoneChanged.bind(this)}, [
          ((EADPI.Time.TimeZones).map((dtz) => (h("option", {"value": dtz, "selected": dtz === this.state.timezone}, `${dtz}`))))
        ])
      ]),
